...
contract BridgeTokens is Script {
    function run(address receiverAddress, uint64 destinationChainSelector, address tokenToSendAddress, uint256 amountToSend, address linkTokenAddress, address routerAddress) public {
        ...
        Client.EVM2AnyMessage memory message = Client.EVM2AnyMessage({
            ...
            extraArgs: Client._argsToBytes(Client.EVMExtraArgsV1({ gasLimit: 0 })) // Default gas limit of 200k
        });
        uint256 ccipFee = IRouterClient(routerAddress).getFee(destinationChainSelector, message);
        IERC20(linkTokenAddress).approve(routerAddress, ccipFee); // LINK token for paying CCIP fees
        IERC20(tokenToSendAddress).approve(routerAddress, amountToSend);  //The ERC20 token you want to bridge
